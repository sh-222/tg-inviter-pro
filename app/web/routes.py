...
from app.core.models import TelegramAccount, TargetUser
...
async def upload_targets(
    ...
    try:
        ...
        return HTMLResponse(
            content=f"<div class='text-red-400 text-sm mt-2'>Error parsing CSV: {str(e)}</div>"
        )
...
async def add_new_account(
    ...
    """
    Endpoint to process new account creation and session file upload.
    """
    ...
    try:
        ...
        account = await TelegramAccount.create(
            ...
            session_string="file",  # We just mark it, we use file-based sessions
        )
        ...
        return f"<div class='text-red-400 text-sm mt-2'>Error creating account: {str(e)}</div>"
